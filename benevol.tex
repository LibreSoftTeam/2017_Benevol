...
Madrid, Spain\\
Email: gerope@libresoft.info}
\and \IEEEauthorblockN{Jesus M. Gonz\'alez-Barahona}
\IEEEauthorblockA{LibreSoft/GSyC\\
Universidad Rey Juan Carlos\\
Madrid, Spain\\
Email: jgb@gsyc.es}
...
%For each ticket we analyzed whether set BIC that we manually founded correspond to the set BIC that the SZZ algorithm founded. We have three outcomes:
%\begin{itemize}
%	\item Set SZZ: When (all) the $ipc$ is(are) the BIC, and furthermore the set of BIC founded manually and using SZZ are equals.
%	\item MIDDLE SZZ: Set SZZ: When a(some) $ipc$ is(are) a BIC and furthermore SZZ algorithm found it(them).
%	\item NO SZZ: When (all) the $ipc$ is(are) not the BIC, and furthermore the intersection of set BIC founded manually and using SZZ is empty.
%\end{itemize}

\section{Results}
\label{sec:results}
%We have analyzed 60 random tickets from the two projects, determining if the ticket presents a BIC and at this case, identifying whether the $ipc$ is(are) the BIC(s). Thus, after finding the lines which fix the bug in the 55 tickets in Nova and 56 tickets in ElasticSearch (we discarded some because we were not sure about the bug fix commit), we classified the tickets in two different sets, SET \textit{``BIC''} and SET \textit{``No BIC''}. In addition, each commit also was classified into the SET of \textit{``FOUNDED''} and \textit{``NO FOUNDED''} by SZZ algorithm. Furthermore, from those classified into the SET \textit{``FOUNDED''} by SZZ, we determined how many of them were classified as \textit{``True possitives''} and \textit{``Flase Positives''} as Table~\ref{tableI} shows.

%From the 55 tickets analyzed in Nova, 73\% was classified into BIC group and 27\% into NO BIC group. On the contrary, from the 56 in ElasticSearch the number of tickets classified into set BIC was lower, 68\%, whereas the set of NO BIC was a little bit higher, 32\%.

%The number of commits analyzed at line level was 121 in Nova and 129 in ElasticSearch, in the 90\% of the cases the SZZ algorithm was able to find them in both projects. But, from the 90\% of commits founded, only in 45\% in Nova and 48\% in ElasticSearch the SZZ algorithm blame the correct $ipc$ as the cause of the bug.

%On the other hand, the number of commits analyzed at token level was 151 in both projects, between the 85-87\% of the cases the SZZ algorithm was able to find them. But, from the 85\% of commits founded, only in 48\% in Nova and 36\% in ElasticSearch the SZZ algorithm blame the correct $ipc$ as the cause of the bug. Some of the $ipc$ were classified into unknown set when we were no sure about whether it was a cause of the bug or not.
...
%In any case, our research shows evidence that assuming that the $ipc$ is where the cause of a bug can be found does not hold for a significant fraction of bugs. The most common reasons for the $ipc$ was not a BIC in the projects are:
%
%\begin{itemize}
